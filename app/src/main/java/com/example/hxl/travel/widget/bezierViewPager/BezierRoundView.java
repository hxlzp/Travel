...
public class BezierRoundView extends View implements ViewPager.OnPageChangeListener {
    ...
    public BezierRoundView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mRadius = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15,
                getResources().getDisplayMetrics());  //默认设置15dp

        /**
         * 获得我们所定义的自定义样式属性
         */
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.BezierRoundView, defStyleAttr, 0);
    ...
    private void startTouchAnimator() {
        //设置触摸范围
        rectF_touch.set(bezPos[nextPos] - mRadius * 1.5f, -mRadius * 1.5f,
        ...
        if (animatorTouch != null) {
            ...
            animatorTouch = ValueAnimator.ofFloat(0, mRadius * 1.5f)
                    .setDuration(time_animator / 2);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isTouchAniming) {  //实现 触摸反馈
            ...
            canvas.drawCircle(bezPos[nextPos], 0, animatedTouchValue, mTouchPaint);

            mTouchPaint.setXfermode(clearXfermode);

            //从 0.7-1.4   效果会更好看！
            canvas.drawCircle(bezPos[nextPos], 0, mRadius * 0.7f, mTouchPaint);

            if (animatedTouchValue >= mRadius) {
                //如果白色的圆半径>=mRadis ，就开始绘制透明的圆
                canvas.drawCircle(bezPos[nextPos], 0,
                        (animatedTouchValue - mRadius) / 0.5f * 1.4f, mTouchPaint);
            }
        ...
        if (disM < animatedValue && animatedValue <= disA) {
            ...
            lRadio = 1.5f - range0Until1(disM, disA) * (1.5f - boundRadio);
            //反弹效果，进场 内弹boundRadio
    ...
    /**
     * @param position       当前cur位置，如果当前是1，手指右滑（vPage向左滑动）那就是0，
     *                       左滑至下一个位置才为2
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        //让 animatedValue 不管是左滑还是右滑，都从[0,1)开始计算
